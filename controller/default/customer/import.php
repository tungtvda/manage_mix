            ...
            if (!file_exists($file)) {
                ...
                if ($total_rows > 2) {
                    for ($row = 2; $row < $total_rows; $row++) {
                        ...
                        $new_customer->phone = $sheet->getCell('E' . $row)->getValue();
                        $new_customer->mobi = $sheet->getCell('F' . $row)->getValue();
                        $new_customer->email = $sheet->getCell('G' . $row)->getValue();
                        $new_customer->director_name = $sheet->getCell('H' . $row)->getValue();
                        $new_customer->company_name = $sheet->getCell('I' . $row)->getValue();
                        $new_customer->chuc_vu = $sheet->getCell('J' . $row)->getValue();
                        $new_customer->note = $sheet->getCell('K' . $row)->getValue();
